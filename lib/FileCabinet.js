 ...
 * NetSuite WSDL is at https://webservices.netsuite.com/wsdl/v2020_1_0/netsuite.wsdl as of this writing
 */
...
function init(env,password) {nsConf = eval(secureConfig.decryptFile(env + "_NetSuiteConfig.js.enc","\"" + password + "\"")); }
 ...
 */
module.exports.postFile = function (filename, description, folder, callback,netsuite_environment, custom_data,password) {
    
    
    if(custom_data){
        console.log(typeof(custom_data))
        console.log(custom_data)
        var customObject = JSON.parse(JSON.stringify(eval("(" + custom_data + ")")))
    }
    
    if(netsuite_environment==undefined){
        return callback("environment")
    }
    init(netsuite_environment,password);
    add(filename, description, folder, customObject,function (err, resp, body) {
        // save the body as a file in the current directory
 ...
 */
function add(filename, description, folder, custom_data,callback) {
    ...
    var nsRequest = makeSoapMsg("addFileTemplate.xml", {
        ...
        content:content,
        description:custom_data ? custom_data.description : '',
        bundle:custom_data ? custom_data.bundle : false,
        online:custom_data ? custom_data.online : false
    });
